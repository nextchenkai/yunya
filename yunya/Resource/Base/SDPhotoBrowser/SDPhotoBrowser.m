...
- (void)setupToolbars
{
    ...
    delButton.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.90f];
...
//    if([_scrollView.subviews[index] isKindOfClass:[UIImageView class]]){
...
//    self.imageCount--;
    ...
    SDBrowserImageView *currentImageView = (SDBrowserImageView *)_scrollView.subviews[index];
...
- (void)deleteAtIndex:(int)index{
    if([self.delegate respondsToSelector:@selector(deleteAtIndex:)]){
        [self.delegate deleteAtIndex:index];
    }
}
...
- (void)layoutSubviews
{
    ...
    _delButton.frame = CGRectMake(screenWidth-55, self.bounds.size.height - 70, 50, 25);
}
